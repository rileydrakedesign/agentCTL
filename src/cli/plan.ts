  ...
  program
    ...
    .action(async (opts) => {
      ...
      if (opts.models) {
        ...
        if (models.length === 0) {
          spinner.fail("--models requires at least one model name.");
          process.exit(1);
        }
        projectConfig.runtime_targets = models.map((model: string) => ({
          model,
          context_window: guessContextWindow(model),
        }));
      }

      // Discover inputs once — needed for both scanning and budget computation
      let inputs: DiscoveryResult;
      try {
      ...
      if (opts.cached) {
        ...
        if (!existsSync(cachedPath)) {
          spinner.fail("No cached scan found. Run `agentctl scan` first.");
          process.exit(1);
        }
        scan = JSON.parse(await readFile(cachedPath, "utf-8"));
        spinner.text = "Using cached scan...";
      } else {
      ...
      // Compute budgets
      spinner.text = "Computing token budgets...";
      ...
      // Exit codes
      if (opts.maxDiscoveryTokens && budgets.discovery_tokens > opts.maxDiscoveryTokens) {
        console.error(
          chalk.red(
            `Discovery tokens (${budgets.discovery_tokens}) exceed budget (${opts.maxDiscoveryTokens})`,
          ),
        );
        process.exit(1);
      }

      if (opts.failOn === "error" && status === "failed") {
        process.exit(1);
      }
      if (opts.failOn === "warning" && (status !== "success" || pressure_warnings.length > 0)) {
        process.exit(1);
      }
    });
}
